...
print('########################################################################')
print('############         Auto NICER Data Reducer V2.0           ############')
print('########################################################################')
print()
...
quantity_support()
time_support()
xti = heasarc.query_object(obj, mission='nicermastr') # calls NICER master catalogue for crab
...
downCommand = ('wget -q -nH --no-check-certificate --cut-dirs=5 -r -l0 -c -N -np -R '+'\'index*\''+' -erobots=off --retr-symlinks https://heasarc.gsfc.nasa.gov/FTP/nicer/data/obs/')
...
for obsid in observations:
	...
	sp.call(str(downCommand) +str(years[count]) +'_'+str(months[count])+'//'+str(obsid)+'/xti/ --show-progress --progress=bar:force', shell=True)
	print('Downloadng log data...')
	sp.call(str(downCommand) +str(years[count]) +'_'+str(months[count])+'//'+str(obsid)+'/log/ --show-progress --progress=bar:force', shell=True)
	print('Downloading auxil data...')
	sp.call(str(downCommand) +str(years[count])+'_'+str(months[count])+'//'+str(obsid)+'/auxil/ --show-progress --progress=bar:force', shell=True)
	sp.call('nicerl2 indir=' + str(obsid) + '/' +' clobber=yes' , shell=True)
	sp.call('barycorr infile=' + str(obsid) + '/xti/event_cl/ni' + str(obsid) + '_0mpu7_cl.evt outfile=' + str(obsid) + '/xti/event_cl/bc' + str(obsid) + 	'_0mpu7_cl.evt orbitfiles=' + str(obsid) + '/auxil/ni' + str(obsid) + '.orb refframe=ICRS ra=' + str(ras[count]) + ' dec=' + str(decs[count]) + ' ephem=JPLEPH.430', shell=True)
...
# Here is the stuff for automatic tar.gz compression
	...
	if q_set == 'y':
		...
		newline = pd.Series(data=[str(base_dir) + '/' + str(obsid) + '/xti/event_cl/bc' + str(obsid) + 	'_0mpu7_cl.evt', 'NI' + str(obsid)], index=['Input', 'Name'])
